...
import { Badge, Button, Section, SectionHeader } from '@/components/ui';
...
export function ProjectsJobsSection() {
  ...
  return (
    <Section variant="gray" padding="xl" className="!px-4">
      <SectionHeader
      ...
      <div className="flex justify-center mb-10">
        <div className="inline-flex p-1.5 rounded-xl flex gap-4">
          <button
          ...
          <button
            ...
            className={cn(
              'flex items-center gap-2 !px-6 !py-3 rounded-lg font-semibold transition-all duration-300 border-2 border-[#1a365d] cursor-pointer hover:bg-[#1a365d] hover:!text-white',
              activeTab === 'jobs'
            ...
            {dummyProjects.map((project, index) => (
              <Link key={project.id} href={`/noi-that/du-an/${project.id}`}>
                <motion.div
                  ...
                  className="group relative rounded-2xl overflow-hidden bg-white shadow-md hover:shadow-xl transition-all duration-300 !p-4"
                >
                  {/* Image */}
                  <div className="relative aspect-[4/3] overflow-hidden">
                    ...
                    {/* Category Badge */}
                    <Badge variant="interior" className="absolute top-4 left-4">
                      {project.category}
                    </Badge>
            ...
            {dummyJobs.map((job, index) => (
              <Link key={job.id} href={`/nhan-su/viec-lam/${job.id}`}>
                <motion.div
                  ...
                  transition={{ delay: index * 0.1 }}
                  className="group flex flex-col md:flex-row md:items-center gap-4 !p-4 bg-white rounded-xl border border-gray-100 hover:shadow-lg hover:border-[#34d399] transition-all duration-300"
                >
